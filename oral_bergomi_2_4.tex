...
\tableofcontents
\newpage
\section{Presentation orale -- Bergomi 2.4 a 2.4.6}

\section{Objectif}

Ce document est concu pour accompagner \textbf{la partie de 10 a 12 minutes} sur :

\begin{itemize}
\item 2.4.1 : volatilite implicite comme moyenne ponderee de volatilite locale,
\item 2.4.2-2.4.4 : approximation "weakly local vol",
\item 2.4.5 : skew, dynamique ATMF, role de \(\alpha(t)\), ratio \(R_T\),
\item 2.4.6 : resultat exact a courte maturite.
\end{itemize}
...
\section{Avant la soutenance : liens explicites avec le cours a maitriser}
...
\begin{enumerate}
\item \textbf{Mesure risque-neutre et dynamique de l'actif}
\end{enumerate}
\begin{itemize}
...
\begin{itemize}
\item savoir distinguer volatilite locale deterministe en \((t,S)\) et volatilite stochastique.
\end{itemize}

\begin{enumerate}
\item \textbf{EDP de pricing / Feynman-Kac}
\end{enumerate}
\begin{itemize}
\item savoir retrouver l'EDP
\end{itemize}
\[
   \partial_t P+(r-q)S\partial_S P+\frac12 \sigma^2 S^2\partial_{SS}P-rP=0 ;
\]
\begin{itemize}
\item comprendre que l'identite de Bergomi repose sur Itô applique a \(e^{-rt}P(t,S_t)\).
\end{itemize}
...
\end{enumerate}
\begin{itemize}
\item savoir rappeler que Dupire reconstruit \(\sigma_{\mathrm{loc}}(T,K)\) a partir de la surface vanille ;
\item expliquer que Bergomi regarde le probleme inverse : que devient l'implicite si on part de \(\sigma_{\mathrm{loc}}\) ?
\end{itemize}

\begin{enumerate}
\item \textbf{Smile, skew, curvature}
\end{enumerate}
\begin{itemize}
\item savoir definir rigoureusement
\end{itemize}
\[
...
\begin{enumerate}
\item \textbf{Dynamique du smile}
\end{enumerate}
\begin{itemize}
\item comprendre la difference entre :
\item mouvement a strike fixe,
\item mouvement ATMF,
\item regimes sticky-strike / sticky-delta.
\end{itemize}

\begin{enumerate}
\item \textbf{Asymptotiques de courte maturite}
\end{enumerate}
\begin{itemize}
\item savoir expliquer pourquoi \(T\to 0\) change la nature de la moyenne ;
\item comprendre la difference entre moyenne temporelle de variances et moyenne spatiale harmonique.
\end{itemize}

\begin{enumerate}
\item \textbf{Calibration statique vs dynamique}
\end{enumerate}
\begin{itemize}
\item point tres classique de cours : un modele peut calibrer la surface a \(t=0\) mais mal decrire sa dynamique future.
\end{itemize}

Si on te pose une question "de cours", les ponts les plus probables sont donc :
\textbf{Itô / Feynman-Kac / Dupire / smile dynamics / asymptotique courte maturite / calibration vs dynamique.}
...
\begin{itemize}
\item "Le point cle ici est qu'on ne cherche plus a calibrer, mais a comprendre la dynamique induite."
\item "Financierement, la vraie question n'est pas seulement le fit du smile aujourd'hui, mais sa reaction demain apres un mouvement de spot."
\end{itemize}
...
\begin{itemize}
\item "Intuitivement, l'option regarde surtout les regions ou son gamma est fort."
\item "Ce que cela signifie financierement, c'est qu'une variance locale situee dans une zone peu visitee ou peu sensible compte peu."
\end{itemize}
...
\begin{itemize}
\item "Le point cle ici est la geometrie des trajectoires : on moyenne sur des ponts lognormaux."
\item "Si je prends seulement \(y=0\), je regarde le chemin le plus probable entre le spot initial et le strike final."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 4 -- Parametrisation locale autour du forward}
...
\begin{itemize}
\item "Intuitivement, \(\alpha(t)\) est la pente locale du smile instantane."
\item "Le point cle ici est que toute la dynamique du smile va etre gouvernee principalement par \(\alpha(t)\)."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 5 -- Le skew implicite est une moyenne ponderee de \(\alpha(t)\)}
...
\begin{itemize}
\item "Pourquoi ce poids \(t/T\) ? Parce qu'au debut de la vie de l'option, le strike terminal ne contraint presque pas encore la trajectoire."
\item "A l'inverse, pres de l'echeance, atteindre le strike final devient essentiel."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 6 -- Mouvement du smile pour un strike fixe quand le spot bouge}
...
\begin{itemize}
\item "Le point cle ici est l'opposition entre deux poids complements : \(t/T\) pour le skew en strike, \(1-t/T\) pour la reaction au spot a strike fixe."
\item "Ce que cela signifie financierement, c'est que le strike fixe et le spot ne sondent pas la meme partie de la structure temporelle du skew local."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 7 -- Dynamique ATMF}
...
\begin{itemize}
\item "Le point cle ici est que toutes les dates comptent pareil."
\item "Financierement, la vol ATMF est entierement pilotee par la courbe de skew local instantane."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 8 -- Le ratio \(R_T\)}
...
\begin{itemize}
\item "Sous local vol, le smile est rigide : quand le spot baisse, la vol ATMF monte souvent trop fortement."
\item "C'est justement l'une des critiques classiques des modeles de volatilite locale sur les sous-jacents equity."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 9 -- Structure par terme du skew : loi de puissance}
...
\begin{itemize}
\item "Intuitivement, l'exposant de decroissance est transmis du local vers l'implicite."
\item "Le point cle ici est que \(\alpha(t)\) ne controle pas seulement le niveau du skew, mais aussi sa structure par terme."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 10 -- Maturite courte : resultat exact}
...
\begin{itemize}
\item "Le point cle ici est que la bonne moyenne a courte maturite est harmonique, pas arithmetique."
\item "Si la local vol s'annule sur une zone a franchir, l'implicite doit aussi tendre vers zero : la formule le respecte naturellement."
\end{itemize}

\bigskip\hrule\bigskip

\section{Slide 11 -- Conclusion, Dupire, stochastic volatility, limites}
...
\begin{itemize}
\item "Ce que cela signifie financierement, c'est qu'un bon fit instantane ne garantit pas une bonne dynamique de smile."
\item "La stochastic volatility est justement introduite pour assouplir cette dynamique."
\end{itemize}

\bigskip\hrule\bigskip

\section{Timing global}
...
\textbf{Total cible : environ 10 min 45 s}

\bigskip\hrule\bigskip

\section{Conseils de tableau}

Ne pas surcharger le tableau. Ecrire seulement :

\begin{enumerate}
\item le SDE local vol au debut ;
\item la formule de moyenne ponderee sur \(\hat{\sigma}^2\) ;
\item la formule du skew implicite
\end{enumerate}
\[
...
\]
\begin{enumerate}
\item la formule de dynamique ATMF
\end{enumerate}
\[
...
\section{Si on me coupe et qu'il faut raccourcir}

Supprimer ou raccourcir :

\begin{itemize}
\item slide 9 (loi de puissance),
\item une partie du detail sur la derivation weak-local-vol,
\item une partie de la conclusion.
\end{itemize}

Garder absolument :

\begin{itemize}
\item moyenne ponderee,
\item role de \(\alpha(t)\),
\item dynamique ATMF,
\item \(R_T\),
\item resultat court terme.
\end{itemize}
\end{document}
